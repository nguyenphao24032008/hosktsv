...
class TechnicModpackService extends AbstractModpackService
{
    ...
    {
       try {
            $response = json_decode($this->client->get('modpack/' . $modpackId, [
                'query' => [
                    'build' => $this->getBuild(),
                ],
            ])->getBody(), true);
            ...
            if ($e instanceof BadResponseException) {
                logger()->error('Received bad response when fetching Technic modpack details.', ['response' => \GuzzleHttp\Psr7\Message::toString($e->getResponse())]);
            }
