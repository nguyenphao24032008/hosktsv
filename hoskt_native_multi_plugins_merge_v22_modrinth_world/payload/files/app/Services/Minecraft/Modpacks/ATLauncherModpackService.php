...
class ATLauncherModpackService extends AbstractModpackService
{
    ...
    {
        $query = 'query { pack(pack: { id: ' . $modpackId . ' }) { name, description, safeName } }';
        ...
        try {
            ...
            if ($e instanceof BadResponseException) {
                logger()->error('Received bad response when fetching ATLauncher modpack details.', ['response' => \GuzzleHttp\Psr7\Message::toString($e->getResponse())]);
            }
        ...
        return [
            ...
            'icon_url' => 'https://cdn.atlcdn.net/images/packs/' . strtolower($pack['safeName']) . '.png',
        ];
    }
}
